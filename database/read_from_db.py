...
def read_query(user: int) -> list:
	"""
	Получаем Id пользователя. Делает запрос к базе данных, получает в ответ результаты запросов данного пользователя
	:param user: int Id пользователя
	:return: list
	"""
	...
	try:
		cursor.execute("SELECT 'id', 'date_time', 'city', 'photo_need' FROM query WHERE 'user_id' = ?", (user,))
		records = cursor.fetchall()
		connect.close()
		return records
	except sqlite3.OperationalError:
		logger.error(f'В базе данных не найдена таблица запросов. User_id : {user}')
		return []
