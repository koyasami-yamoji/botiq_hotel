...
def get_list_history(message) -> None:
	...
	if queries:
		...
		for item in queries:
			bot.send_message(message.chat.id, f'({item[0]}). Дата и время {item[1]}. Введенный город {item[2]}')
		bot.set_state(message.chat.id, HotelInfoState.select_number)
		bot.send_message(message.chat.id, f'Введите номер интересующего вас варианта: ')
	else:
		bot.send_message(message.chat.id, f'В истории поиска нечего не найдено...')


@bot.message_handler(state=HotelInfoState.select_number)
